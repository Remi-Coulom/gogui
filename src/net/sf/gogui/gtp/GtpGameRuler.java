...
/** Interface to a Game ruler program that uses GTP over the standard I/O streams.
    <p>
...
public final class GtpGameRuler
extends GtpClientBase
{
    ...
    /** Callback if a timeout occurred. */
    ...
    /** Callback if an invalid response occurred.
    ...
    public GtpGameRuler(String program, File workingDirectory, boolean log,
            IOCallback callback/*, GtpSynchronizer synchronizer*/)
                    throws GtpGameRuler.ExecFailed
    {
    ...
    public GtpGameRuler(InputStream in, OutputStream out, boolean log,
            IOCallback callback/*, GtpSynchronizer synchronizer*/)
                    throws GtpError
    {
        m_log = log;
    ...
    {
        String res = send(command, -1, null);
        return res;
    }
    ...
    public void setIOCallback(GtpGameRuler.IOCallback callback)
    {
        m_callback = callback;
    }
    ...
    public String getCommandLegalMoves() {
        return "gogui-rules_legal_moves";
    }

    public GoColor getSideToMove(Move move) throws GtpError {
        if (! isSupported("gogui-rules_side_to_move"))
            return move.getColor().otherColor();
        String color = send("gogui-rules_side_to_move");
    ...
    public boolean isGameOver() throws GtpError {
        if (! isSupported("gogui-rules_legal_moves"))
            return false;
        return getLegalMoves().equals("");
    }

    /**
     * "pass" at the end if pass move is possible
     */
    public boolean isLegalMove(Move move) throws GtpError
    {
        if (! isSupported("gogui-rules_legal_moves"))
            return false;
    ...
    public String getLegalMoves() throws GtpError
    {
        return send("gogui-rules_legal_moves");
    }
     ...
     * are "pass" and "PASS"
     */
    public boolean isPassLegal() throws GtpError
    {
        if (! isSupported("gogui-rules_legal_moves"))
            return false;
        String legalMoves = getLegalMoves();
